...
impl EthStateCache {
    ...
    pub async fn get_sealed_block(&self, block_hash: B256) -> ProviderResult<Option<SealedBlock>> {
        Ok(self
            ...
            .await?
            .map(|block| block.seal(block_hash)))
    }
    ...
    pub async fn get_block_transactions(
        ...
        let (response_tx, rx) = oneshot::channel();
        let _ = self.to_service.send(CacheAction::GetBlockTransactions {
            block_hash,
            response_tx,
        });
        rx.await
    ...
    pub async fn get_evm_env(
        ...
        let (response_tx, rx) = oneshot::channel();
        let _ = self.to_service.send(CacheAction::GetEnv {
            block_hash,
            response_tx,
        });
        rx.await
            ...
            match ready!(this.action_rx.poll_next_unpin(cx)) {
                ...
                Some(action) => {
                    match action {
                                ...
                                .queue(block_hash, Either::Left(response_tx))
                            {
                                ...
                                this.action_task_spawner
                                    .spawn_blocking(Box::pin(async move {
                                        ...
                                        let _ =
                                            action_tx.send(CacheAction::BlockWithSendersResult {
                                                block_hash,
                                                res: block_sender,
                                            });
                                    }));
                        ...
                        CacheAction::GetBlockTransactions {
                            block_hash,
                            response_tx,
                        } => {
