...
use scalar_consensus::ConsensusEngineHandle;
    ...
    Provider: HeaderProvider + BlockReader + StateProviderFactory + EvmEnvProvider + 'static,
{
    ...
    async fn validate_and_execute_forkchoice(
        ...
        Ok(self
            .inner
            .beacon_consensus
            .fork_choice_updated(state, payload_attrs)
            .await?)
    }
}
...
mod tests {
    ...
    mod get_payload_bodies {
        ...
        async fn returns_payload_bodies() {
            ...
            handle
                .provider
                .extend_blocks(blocks.iter().cloned().map(|b| (b.hash(), b.unseal())));
    ...
    mod exchange_transition_configuration {
        ...
        async fn terminal_td_mismatch() {
            ...
            let res = api
                .exchange_transition_configuration(transition_config.clone())
                .await;
        ...
        async fn configurations_match() {
            ...
            handle
                .provider
                .add_block(terminal_block.hash(), terminal_block.unseal());
