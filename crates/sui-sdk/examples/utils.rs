...
use sui_config::{
    scalar_config_dir, Config, PersistedConfig, SCALAR_KEYSTORE_FILENAME, SUI_CLIENT_CONFIG,
};
...
pub async fn split_coin_digest(
    ...
    let keystore = FileBasedKeystore::new(&scalar_config_dir()?.join(SCALAR_KEYSTORE_FILENAME))?;
...
pub async fn retrieve_wallet() -> Result<WalletContext, anyhow::Error> {
    let wallet_conf: std::path::PathBuf = scalar_config_dir()?.join(SUI_CLIENT_CONFIG);
