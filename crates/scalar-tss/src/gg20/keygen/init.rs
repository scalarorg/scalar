...
//! A [KeygenInitSanitized] struct is created out of the raw incoming [narwhal_types::KeygenInit] message and a key is reserved inside the KvStore
//! If [narwhal_types::KeygenInit] fails to be parsed, an [InitResult] is returned
...
) -> anyhow::Result<(usize, Vec<String>, Vec<usize>)> {
    // save my uid
    let my_uid = uids
        .get(my_index)
