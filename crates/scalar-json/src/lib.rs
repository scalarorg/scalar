...
use scalar_types::id::{ID, RESOLVED_SUI_ID};
use scalar_types::move_package::MovePackage;
use scalar_types::transfer::RESOLVED_RECEIVING_STRUCT;
use scalar_types::MOVE_STDLIB_ADDRESS;
...
macro_rules! call_arg {
    ($value:expr) => {{
        ...
        impl SuiJsonArg for scalar_types::base_types::ObjectID {
            fn to_sui_json(&self) -> anyhow::Result<SuiJsonValue> {
                SuiJsonValue::from_str(&self.to_string())
            }
        }
        impl SuiJsonArg for scalar_types::base_types::SuiAddress {
            fn to_sui_json(&self) -> anyhow::Result<SuiJsonValue> {
                SuiJsonValue::from_str(&self.to_string())
            }
        }
...
macro_rules! type_args {
    ($($value:expr), *) => {{
        use scalar_json_rpc_types::SuiTypeTag;
        ...
        impl <T: core::fmt::Display> SuiJsonTypeArg for T {
            fn to_sui_json(&self) -> anyhow::Result<SuiTypeTag> {
                Ok(scalar_types::parse_sui_type_tag(&self.to_string())?.into())
            }
        }
