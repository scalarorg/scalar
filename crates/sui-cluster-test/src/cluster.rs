...
use sui_config::{PersistedConfig, SCALAR_KEYSTORE_FILENAME, SCALAR_NETWORK_CONFIG};
...
impl Cluster for LocalNewCluster {
    async fn start(options: &ClusterTestOpt) -> Result<Self, anyhow::Error> {
        ...
        if let Some(config_dir) = options.config_dir.clone() {
            ...
            let network_config_path = config_dir.join(SCALAR_NETWORK_CONFIG);
...
pub async fn new_wallet_context_from_cluster(
    ...
    let keystore_path = config_dir.join(SCALAR_KEYSTORE_FILENAME);
