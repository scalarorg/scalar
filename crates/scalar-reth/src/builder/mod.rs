...
impl NodeConfig {
    ...
    pub async fn launch<E: RethCliExt>(
        ...
        match db_instance {
            DatabaseInstance::Real { db, data_dir } => {
                let builder = NodeBuilderWithDatabase {
                    config: self,
                    db,
                    data_dir,
                };
        ...
        Client: HeadersClient,
    {
        let max_block = if let Some(block) = self.debug.max_block {
            ...
            Some(
                self.lookup_or_fetch_tip(provider_factory, network_client, tip)
                    .await?,
            )
        } else {
            None
    ...
    pub async fn build_network<DB>(
        ...
        ProviderFactory<DB>,
        NetworkBuilder<ProviderFactory<DB>, (), ()>,
    )>
    where
    ...
    {
        Ok(self
            .fetch_tip(provider_factory, client, BlockHashOrNumber::Hash(tip))
            .await?
            .number)
    }
...
impl<DB: Database + DatabaseMetrics + DatabaseMetadata + 'static> NodeBuilderWithDatabase<DB> {
    ...
    pub async fn launch<E: RethCliExt>(
        ...
        let transaction_pool =
            self.config
                .build_and_spawn_txpool(&blockchain_db, head, &executor, &self.data_dir)?;
        ...
        let max_block = self
            .config
            .max_block(&network_client, provider_factory.clone())
            .await?;

        // Configure the pipeline
        let (mut pipeline, client) = if self.config.dev.dev {
            info!(target: "reth::cli", "Starting Reth in dev mode");
            let mining_mode = self
                .config
                .mining_mode(transaction_pool.pending_transactions_listener());
        ...
        // Start RPC servers
        let rpc_server_handles = self
            .config
            .rpc
            .start_servers(&components, engine_api, jwt_secret, &mut ext)
            .await?;
    ...
    fn config_path(&self) -> PathBuf {
        self.config
            .config
            .clone()
            .unwrap_or_else(|| self.data_dir.config_path())
    }
...
mod tests {
    ...
    async fn block_number_node_config_test() {
        ...
        let handle = spawn_node(NodeConfig::test().with_rpc(rpc_args).with_instance(1))
            .await
            .unwrap();
    ...
    async fn rpc_handles_none_without_http() {
        ...
        let handle = spawn_node(NodeConfig::test().with_instance(2))
            .await
            .unwrap();
    ...
    async fn launch_multiple_nodes() {
        ...
        for i in 0..num_nodes {
            let handle = spawn_node(NodeConfig::test().with_instance(starting_instance + i))
                .await
                .unwrap();
            handles.push(handle);
        }
