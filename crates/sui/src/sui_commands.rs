...
use sui_config::{
    scalar_config_dir, Config, PersistedConfig, FULL_NODE_DB_PATH, SUI_CLIENT_CONFIG,
    SUI_FULLNODE_CONFIG, SUI_NETWORK_CONFIG,
};
use sui_config::{
    SUI_BENCHMARK_GENESIS_GAS_KEYSTORE_FILENAME, SUI_GENESIS_FILENAME, SCALAR_KEYSTORE_FILENAME,
};
    ...
    pub async fn execute(self) -> Result<(), anyhow::Error> {
        ...
        match self {
            SuiCommand::Start {
                ...
                if config.is_none() && !scalar_config_dir()?.join(SUI_NETWORK_CONFIG).exists() {
                ...
                let network_config_path = config
                    .clone()
                    .unwrap_or(scalar_config_dir()?.join(SUI_NETWORK_CONFIG));
            ...
            SuiCommand::Network {
                ...
                let config_path = config.unwrap_or(scalar_config_dir()?.join(SUI_NETWORK_CONFIG));
            ...
            SuiCommand::KeyTool {
                ...
                let keystore_path =
                    keystore_path.unwrap_or(scalar_config_dir()?.join(SCALAR_KEYSTORE_FILENAME));
                let mut keystore = Keystore::from(FileBasedKeystore::new(&keystore_path)?);
                cmd.execute(&mut keystore).await?.print(!json);
                Ok(())
            }
            SuiCommand::Console { config } => {
                let config = config.unwrap_or(scalar_config_dir()?.join(SUI_CLIENT_CONFIG));
            ...
            SuiCommand::Client {
                ...
                let config_path = config.unwrap_or(scalar_config_dir()?.join(SUI_CLIENT_CONFIG));
...
async fn genesis(
    ...
    let scalar_config_dir = &match working_dir {
        ...
        // yet
        None => {
            let config_path = scalar_config_dir()?;
    ...
    // error
    let dir = scalar_config_dir.read_dir().map_err(|err| {
        anyhow!(err).context(format!("Cannot open Sui config dir {:?}", scalar_config_dir))
    })?;
    let files = dir.collect::<Result<Vec<_>, _>>()?;

    let client_path = scalar_config_dir.join(SUI_CLIENT_CONFIG);
    let keystore_path = scalar_config_dir.join(SCALAR_KEYSTORE_FILENAME);
    ...
    if write_config.is_none() && !files.is_empty() {
        if force {
            ...
            if is_compatible {
                ...
                fs::remove_dir_all(scalar_config_dir).map_err(|err| {
                    anyhow!(err)
                        .context(format!("Cannot remove Sui config dir {:?}", scalar_config_dir))
                })?;
                fs::create_dir(scalar_config_dir).map_err(|err| {
                    anyhow!(err)
                        .context(format!("Cannot create Sui config dir {:?}", scalar_config_dir))
                })?;
            }
        } else if files.len() != 2 || !client_path.exists() || !keystore_path.exists() {
            bail!("Cannot run genesis with non-empty Sui config directory {}, please use the --force/-f option to remove the existing configuration", scalar_config_dir.to_str().unwrap());
        }
    }

    let network_path = scalar_config_dir.join(SUI_NETWORK_CONFIG);
    let genesis_path = scalar_config_dir.join(SUI_GENESIS_FILENAME);
    ...
    let mut genesis_conf = match from_config {
        ...
        None => {
            if let Some(ips) = benchmark_ips {
                ...
                let path = scalar_config_dir.join(SUI_BENCHMARK_GENESIS_GAS_KEYSTORE_FILENAME);
    ...
    let builder = ConfigBuilder::new(scalar_config_dir);
    ...
    fullnode_config.save(scalar_config_dir.join(SUI_FULLNODE_CONFIG))?;
    let mut ssfn_nodes = vec![];
    if let Some(ssfn_info) = ssfn_info {
        for (i, ssfn) in ssfn_info.into_iter().enumerate() {
            let path =
                scalar_config_dir.join(sui_config::ssfn_config_file(ssfn.p2p_address.clone(), i));
        ...
        for (i, mut validator) in network_config
            ...
            .enumerate()
        {
            let path = scalar_config_dir.join(sui_config::validator_config_file(
    ...
    if !wallet_conf_path.exists() {
        ...
        if let Some(env) = env {
            let keystore_path = wallet_conf_path
                .parent()
                .unwrap_or(&scalar_config_dir()?)
                .join(SCALAR_KEYSTORE_FILENAME);
            let mut keystore = Keystore::from(FileBasedKeystore::new(&keystore_path)?);
