...
use sui_config::{Config, SCALAR_NETWORK_CONFIG, SUI_CLIENT_CONFIG};
use sui_config::{NodeConfig, PersistedConfig, SCALAR_KEYSTORE_FILENAME};
...
impl TestClusterBuilder {
    ...
    async fn start_swarm(&mut self) -> Result<Swarm, anyhow::Error> {
        ...
        let network_path = dir.join(SCALAR_NETWORK_CONFIG);
        let wallet_path = dir.join(SUI_CLIENT_CONFIG);
        let keystore_path = dir.join(SCALAR_KEYSTORE_FILENAME);
