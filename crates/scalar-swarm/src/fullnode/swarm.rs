...
pub struct FullnodeSwarmBuilder<R = OsRng> {
    rng: R,
    // template: NodeConfig,
    dir: Option<PathBuf>,
...
impl<R> FullnodeSwarmBuilder<R> {
    pub fn rng<N: rand::RngCore + rand::CryptoRng>(self, rng: N) -> FullnodeSwarmBuilder<N> {
        FullnodeSwarmBuilder {
            rng,
            dir: self.dir,
...
impl<R: rand::RngCore + rand::CryptoRng> FullnodeSwarmBuilder<R> {
    /// Create the configured Swarm.
    pub fn build(self) -> FullnodeSwarm {
        ...
        if self.fullnode_count > 0 {
            (0..self.fullnode_count).for_each(|idx| {
                ...
                nodes.insert(config.protocol_public_key(), FullNode::new(config));
            });
        }
        FullnodeSwarm {
            dir,
...
impl FullnodeSwarm {
    fn nodes_iter_mut(&mut self) -> impl Iterator<Item = &mut FullNode> {
        self.nodes.values_mut()
    }

    /// Return a new Builder
    pub fn builder() -> FullnodeSwarmBuilder {
        FullnodeSwarmBuilder::new()
    }
    ...
    pub fn all_nodes(&self) -> impl Iterator<Item = &FullNode> {
        self.nodes.values()
    }

    pub fn node(&self, name: &AuthorityName) -> Option<&FullNode> {
        self.nodes.get(name)
    }

    pub fn node_mut(&mut self, name: &AuthorityName) -> Option<&mut FullNode> {
    ...
    pub fn validator_nodes(&self) -> impl Iterator<Item = &FullNode> {
        self.nodes
            .values()
    ...
    pub fn active_validators(&self) -> impl Iterator<Item = &FullNode> {
    ...
    pub fn fullnodes(&self) -> impl Iterator<Item = &FullNode> {
        self.nodes
            .values()
