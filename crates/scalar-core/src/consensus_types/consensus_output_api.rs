    ...
    fn transactions(&self) -> ConsensusOutputTransactions {
        ...
        self.sub_dag
            ...
            .map(|(cert, batches)| {
                ...
                let transactions: Vec<(&[u8], ConsensusTransactionWrapper)> = cert.header().system_messages().iter().filter_map(|msg| {
                    // Generate transactions to write new randomness.
                    if let SystemMessage::RandomnessSignature(round, bytes) = msg {
                        Some(([0u8; 0].as_slice(), ConsensusTransactionWrapper::Consensus(ConsensusTransaction {
                            tracking_id: [0; 8],
